package com.smjcco.wxpusher.client.sdk;

import com.smjcco.wxpusher.client.sdk.bean.*;
...
public class ClientTest {
    ...
    private static final String TEST_APP_TOKEN = "AT_xxx"; // 测试用的APP_TOKEN，请替换为自己的
    ...
    public void setup() {
        // 也可以使用静态方法初始化默认实例
        WxPusher.initDefaultWxPusher(TEST_APP_TOKEN);
    }
    ...
    public void testQueryMessageStatus() {
        ...
        Result<List<MessageResult>> sendResult = WxPusher.getDefaultWxPusher().send(message);
        if (sendResult.isSuccess() && sendResult.getData() != null && !sendResult.getData().isEmpty()) {
            ...
            if (messageId != null) {
                Result<Integer> result = WxPusher.getDefaultWxPusher().queryMessageStatus(messageId);
    ...
    public void testQuerySpecificUser() {
        Result<Page<WxUser>> result = WxPusher.getDefaultWxPusher().queryWxUserV2(1, 10, TEST_UID, false, UserType.APP);
    ...
    public void testQueryScanUID() {
        ...
        Result<CreateQrcodeResp> createResult = WxPusher.getDefaultWxPusher().createAppTempQrcode(req);
    ...
    public void testDeleteUser() {
        // 先查询出用户ID
        Result<Page<WxUser>> queryResult = WxPusher.getDefaultWxPusher().queryWxUserV2(1, 10, TEST_UID, false, UserType.APP);
